...
public class MobileAuthFilter extends OncePerRequestFilter {
    ...
    private Optional<UserEntity> validate(HttpServletRequest request) {
        ...
        return authService.checkCustomerToken(token);
    }
    @Override
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        var authentication = new UsernamePasswordAuthenticationToken(
                user,
                null,
                Collections.singletonList(new SimpleGrantedAuthority("LOCAL_CUSTOMER"))
        );
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        return !request.getServletPath().startsWith("/mobile");
    }
}
